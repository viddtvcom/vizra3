        ...
        switch (true) {
            ...
            case is_object($val):
                ...
                foreach (get_object_vars($val) as $k => $v) {
                    $pXml = isset($pXml) ? $pXml . $this->serialize_val(
                        $v,
                        $k,
                        false,
                        false,
 ...
 * @access   public
 */
function timestamp_to_iso8601_di($timestamp, $utc = true)
{
 ...
 * @access   public
 */
function iso8601_to_timestamp_di($datestr)
{
    $eregStr =
            '([0-9]{4})-' . // centuries & years CCYY-
                    '([0-9]{2})-' . // months MM-
...
function usleepWindows_di($usec)
{
    $start = gettimeofday();
 ...
 * @access public
 */
class soap_fault_di extends nusoap_base_di
{
     ...
     */
    function soap_fault_di($faultcode, $faultactor = '', $faultstring = '', $faultdetail = '')
    {
 ...
 * @access   public
 */
class XMLSchema_di extends nusoap_base_di
{
     ...
     * @access   public
     */
    function XMLSchema_di($schema = '', $xml = '')
    {
...
class soapval_di extends nusoap_base_di
{
    /**
     * constructor
     *
 ...
 * @access public
 */
class soap_transport_http_di extends nusoap_base_di
{
     ...
     * constructor
     */
    function soap_transport_http_di($url)
    {
        $this->url = $url;
    ...
    function connect($connection_timeout = 0, $response_timeout = 30)
    {
        if ($this->scheme == 'http') {
            ...
            if (! $this->fp) {
                $this->debug(
                    'Couldn\'t open socket connection to server ' . $this->url . ', Error: ' . $this->error_str
                );
                $this->setError(
                    'Couldn\'t open socket connection to server: ' . $this->url . ', Error: ' . $this->error_str
                );
...
//curl_setopt($this->ch, CURLOPT_CAINFO, 'f:\php-4.3.2-win32\extensions\curl-ca-bundle.crt');        
    ...
    function getResponse()
    {
        ...
        if ($this->scheme == 'http') {
            ...
            $strlen = 0;
            while ((isset($this->incoming_headers['content-length']) && $strlen < $this->incoming_headers['content-length']) || ! feof(
                $this->fp
            )) {
            ...
            if (strtolower($this->incoming_headers['content-encoding']) == 'deflate' || strtolower(
                ...
                if (function_exists('gzuncompress')) {
                    ...
                    if ($this->incoming_headers['content-encoding'] == 'deflate' && $degzdata = @gzuncompress($data)) {
                        $data = $degzdata;
                    } elseif ($this->incoming_headers['content-encoding'] == 'gzip' && $degzdata = gzinflate(
                        substr($data, 10)
                    )
                    ) { // do our best
                        $data = $degzdata;
 ...
 * @access   public
 */
class soap_server_di extends nusoap_base_di
{
     ...
     * @access   public
     */
    function soap_server_di($wsdl = false)
    {
    ...
    function parse_request($data = '')
    {
            ...
            if ($this->headers['Content-Encoding'] == 'deflate' || $this->headers['Content-Encoding'] == 'gzip') {
                ...
                if (function_exists('gzuncompress')) {
                    if ($this->headers['Content-Encoding'] == 'deflate' && $degzdata = @gzuncompress($data)) {
                        $data = $degzdata;
                    } elseif ($this->headers['Content-Encoding'] == 'gzip' && $degzdata = gzinflate(
                        substr($data, 10)
                    )
                    ) {
                        $data = $degzdata;
        ...
        $parser = new soap_parser_di($data, $this->xml_encoding);
        ...
        if ($err = $parser->getError()) {
            ...
            if ($this->verify_method($this->methodname, $request_data)) {
                ...
                $this->debug(
                    "done calling method: $this->methodname, received $method_response of type" . gettype(
                        $method_response
                    )
                );
    ...
    {
        $this->fault = new soap_fault_di($faultcode, $faultactor, $faultstring, $faultdetail);
    }
 ...
 * @access public
 */
class wsdl_di extends XMLSchema_di
{
    var $wsdl;
    // define internal arrays of bindings, ports, operations, messages, etc.
    var $message = array();
     ...
     * @access public
     */
    function wsdl_di($wsdl = '', $proxyhost = false, $proxyport = false, $proxyusername = false, $proxypassword = false)
    {
        $this->wsdl = $wsdl;
    ...
    function parseWSDL($wsdl = '')
    {
        ...
        if (isset($wsdl_props['host'])) {
            ...
            // get wsdl
            $tr = new soap_transport_http_di($wsdl);
            ...
            switch ($this->status) {
                case 'message':
                    if ($name == 'part') {
                        if (isset($attrs['type'])) {
                            $this->debug(
                                "msg " . $this->currentMessage . ": found part $attrs[name]: " . implode(',', $attrs)
                            );
                ...
                case 'service':
                    switch ($name) {
                        ...
                        case 'address':
                            ...
                            //global $SERVICE_URL;   // This line is added By Manish.

                            //$serviceurl= "https://api.onlyfordemo.net/anacreon/servlet/APIv3";
                            //$serviceurl = "https://www.foundationapi.com/anacreon/servlet/APIv3";

                            $this->ports[$this->currentPort]['location'] = $GLOBALS["soap_serviceurl"]; //This line is added By Manish
                            ...
                            $this->bindings[$this->ports[$this->currentPort]['binding']]['endpoint'] = $GLOBALS["soap_serviceurl"]; //This line is added By Manish
                            break;
                    }
                    break;
            }
            ...
            foreach ($this->messages as $msgName => $msgParts) {
                ...
                if (is_array($msgParts)) {
                    foreach ($msgParts as $partName => $partType) {
                        ...
                        $xml .= '<part name="' . $partName . '" type="' . $typePrefix . ':' . $this->getLocalPart(
                            $partType
                        ) . '" />';
                    }
                }
            ...
            foreach ($this->bindings as $bindingName => $attrs) {
                ...
                foreach ($attrs['operations'] as $opName => $opParts) {
                    ...
                    if (isset($opParts['documentation']) && $opParts['documentation'] != '') {
                        $portType_xml .= '<documentation>' . htmlspecialchars(
    ...
    {
        $this->debug(
            'in serializeRPCParameters with operation ' . $operation . ', direction ' . $direction . ' and ' . count(
                $parameters
    ...
    {
        $this->debug(
            'in serializeParameters with operation ' . $operation . ', direction ' . $direction . ' and ' . count(
                $parameters
            ) . ' param(s), and xml schema version ' . $this->XMLSchemaVersion
        );
    ...
    function serializeType($name, $type, $value, $use = 'encoded', $encodingStyle = false)
    {
        ...
        if (strpos($type, ':')) {
            ...
            if ($ns == $this->XMLSchemaVersion ||
                ...
                if ($use == 'literal') {
                    ...
                    return "<$name xsi:type=\"" . $this->getPrefixFromNamespace(
                    ...
                    if ($value) {
                        foreach ($value as $k => $v) {
                            ...
                            $contents .= $this->serialize_val(
                                ...
                                $use,
                                $uqType
                            ); //This para is added By Manish $uqType
                            $contents .= '</item>';
                        }
                    }
                    if ($use == 'literal') {
        ...
        $this->debug(
            "serializeType: uqType: $uqType, ns: $ns, phptype: $phpType, arrayType: " . (isset($typeDef['arrayType']) ? $typeDef['arrayType'] : '')
        );
        ...
        if ($phpType == 'struct') {
            ...
            if ($use == 'literal') {
                ...
                $xml = "<$elementName$elementNS xsi:type=\"" . $this->getPrefixFromNamespace(
                    $ns
                    ...
                    for ($i = 0; $i < $p; $i ++) {
                        ...
                        if (isset($optionals) && ! isset($value[$eName])) {
                            ...
                            if ($this->getTypeDef($this->getLocalPart($attrs['type']))) {
                                ...
                                $this->debug(
                                    "calling serialize_val() for $eName, $v, " . $this->getLocalPart($attrs['type']),
                                    false,
                                    $use
                                );
            ...
            if ($use == 'literal') {
                ...
                $xml = "<$name xsi:type=\"" . $this->getPrefixFromNamespace(
    ...
    function addOperation(
        ...
        if ($in) {
            foreach ($in as $pName => $pType) {
                if (strpos($pType, ':')) {
                    $pType = $this->getNamespaceFromPrefix($this->getPrefix($pType)) . ":" . $this->getLocalPart(
                        $pType
                    );
                }
 ...
 * @access   public
 */
class soap_parser_di extends nusoap_base_di
{
     ...
     * @access   public
     */
    function soap_parser_di($xml, $encoding = 'UTF-8', $method = '')
    {
        $this->xml = $xml;
        ...
        if (! empty($xml)) {
            $this->debug('Entering soap_parser_di(), length=' . strlen($xml) . ', encoding=' . $encoding);
    ...
    function buildVal($pos)
    {
        ...
        $this->debug(
            'inside buildVal() for ' . $this->message[$pos]['name'] . "(pos $pos) of type " . $this->message[$pos]['type']
        );
...
/**
 *
 * soap2client higher level class for easy usage.
 *
 * usage:
 *
 * // instantiate client with server info
 * $soap2client = new soap2client( string path [ ,boolean wsdl] );
 *
 * // call method, get results
 * echo $soap2client->call( string methodname [ ,array parameters] );
 *
 * // bye bye client
 * unset($soap2client);
 *
 ...
 * @access   public
 */
class soap2client extends nusoap_base_di
{
    ...
    function soap2client(
        ...
        if ($wsdl) {
            ...
            $this->wsdl =& new wsdl_di($this->wsdlFile, $this->proxyhost, $this->proxyport, $this->proxyusername, $this->proxypassword);
        ...
        switch (true) {
            ...
            case ereg('^http', $this->endpoint):
                ...
                if ($this->persistentConnection == true && is_object($this->persistentConnection)) {
                    ...
                    $http = new soap_transport_http_di($this->endpoint);
                ...
                if ($err = $http->getError()) {
                    ...
                    $this->debug(
                        'got response, length: ' . strlen($this->responseData) . ' use encoding: ' . $this->xml_encoding
                    );
    ...
    function parseResponse($data)
    {
        ...
        $parser = new soap_parser_di($data, $this->xml_encoding, $this->operation);
        // if parse errors
    ...
    function getProxy()
    {
        ...
        $r = rand();
        $evalStr = 'class soap_proxy_' . $r . ' extends soap2client {
                ' . $evalStr . '
            }';
