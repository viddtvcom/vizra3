...
class MSN
{
    ...
    function MSN($protocol = '', $debug = false, $timeout = 15, $client_id = 0x7000800C)
    {
        ...
        if ($protocol != 'MSNP9' && ! function_exists('mcrypt_cbc')) {
            if ($protocol == 'MSNP15') {
                die("We need mcrypt module for $protocol!\n");
            }
            $protocol = 'MSNP9';
        }
        if ($protocol != 'MSNP9' && ! function_exists('bcmod')) {
            if ($protocol == 'MSNP15') {
                die("We need bcmath module for $protocol!\n");
            }
            $protocol = 'MSNP9';
        }
        if ($protocol == 'MSNP9') {
    ...
    function get_UUID()
    {
        ...
        return substr($workid, 0, 8) . '-' . substr($workid, 8, 4) . '-' . substr($workid, 12, 4) . '-' . substr(
            $workid,
            16,
            4
        ) . '-' . substr($workid, 20, 12);
...
<soap:Body>
    <ABContactAdd xmlns="http://www.msn.com/webservices/AddressBook">
        ...
        if ($http_code != 200) {
            ...
            $this->log_message(
                "*** can't add contact (network: $network) $email, error code: $faultcode, $faultstring"
            );
            return false;
        }
    ...
    <DeleteMember xmlns="http://www.msn.com/webservices/AddressBook">
        ...
        if ($http_code != 200) {
            ...
            if (strcasecmp($faultcode, 'soap:Client') || stripos($faultstring, 'Member does not exist') === false) {
                $this->log_message(
                    "*** can't delete member (network: $network) $email ($memberID) to $list, error code: $faultcode, $faultstring"
                );
                return false;
            }
    ...
    <AddMember xmlns="http://www.msn.com/webservices/AddressBook">
        ...
        if ($http_code != 200) {
            ...
            if (strcasecmp($faultcode, 'soap:Client') || stripos($faultstring, 'Member already exists') === false) {
                $this->log_message(
                    "*** can't add member (network: $network) $email to $list, error code: $faultcode, $faultstring"
                );
                return false;
            }
...
<soap:Body>
    <FindMembership xmlns="http://www.msn.com/webservices/AddressBook">
        ...
        foreach ($aMemberships as $sMembership) {
            ...
            if ($sMemberRole != 'Allow' && $sMemberRole != 'Reverse' && $sMemberRole != 'Pending') {
                continue;
            }
            $p = $sMembership;
            if (isset($aMembers)) {
            ...
            foreach ($aMembers as $sMember) {
                ...
                if ($sMemberType == 'PassportMember') {
                    if (strpos($sMember, '<Type>Passport</Type>') === false) {
                        continue;
                    }
                    $network = 1;
                    ...
                    preg_match(
                        '#<Annotation><Name>MSN.IM.BuddyType</Name><Value>(.*):(.*)</Value></Annotation>#',
                        $sMember,
                        $matches
                    );
                    if (count($matches) == 0) {
                ...
                if (count($matches) > 0) {
                    ...
                    $this->log_message(
                        "*** add new contact (network: $network, status: $sMemberRole): $u_name@$u_domain ($id)"
                    );
                }
            }
        }
    ...
    function connect($user, $password, $redirect_server = '', $redirect_port = 1863)
    {
        if ($this->protocol == 'MSNP9') {
            $this->error = 'MSN Botu kullanabilmeniz için mcrypt ve bcmath PHP kütüphaneleri yüklü olmalıdır';
            return false;
        }
        $this->id = 1;
        if ($redirect_server === '') {
        ...
        while (1) {
            ...
            if (! is_resource($this->fp) || feof($this->fp)) {
                ...
                if ($this->protocol == 'MSNP9') {
                    ...
                    // NS: >>> UUX {id} length
                    $str = '<Data><PSM>' . htmlspecialchars(
                        $psm
                    ) . '</PSM><CurrentMedia></CurrentMedia><MachineGuid></MachineGuid></Data>';
                    $len = strlen($str);
            ...
            if ($data === false) {
                ...
                $aFiles = glob(
                    dirname($_SERVER['argv'][0]) . DIRECTORY_SEPARATOR . 'spool' . DIRECTORY_SEPARATOR . '*.msn'
                );
                if (! is_array($aFiles)) {
                    continue;
                }
                ...
                foreach ($aFiles as $filename) {
                    if (fileperms($filename) != (0x8000 | 00666)) {
                        continue;
                    }
                    ...
                    if (! is_array($aTo) || count($aTo) == 0 || $sMessage === '' || $to_str === '') {
                        ...
                        if ($backup_file) {
                            ...
                            $backup_name = $backup_dir . '/' . strftime('%Y%m%d%H%M%S') . '_' . $this->getpid(
                            ) . '_' . basename($filename);
                ...
                if ($nMSNUsers == 0) {
                    ...
                    if ($this->protocol == 'MSNP9') {
                        $this->debug_message(
                            "*** MSNP9 don't support other network, so we ignore other network user!"
                        );
                    } else {
            ...
            switch ($code) {
                ...
                case 'ADD':
                    ...
                    if (isset($aContactList[$u_domain][$u_name][1]['Allow'])) {
                        $this->log_message(
                            "*** someone add us to their list (but already in our list): $u_name@$u_domain"
                        );
                    } else {
                        $aContactList[$u_domain][$u_name][1]['Allow'] = 'Allow';
                ...
                case 'REM':
                    ...
                    if (isset($aContactList[$u_domain][$u_name][1])) {
                        ...
                        $this->log_message(
                            "*** someone remove us from their list (but not in our list): $u_name@$u_domain"
                        );
                    }
                    ...
                    if (is_numeric($size) && $size > 0) {
                        ...
                        if (is_array($matches) && count($matches) > 0) {
                            ...
                            if (isset($aContactList[$u_domain][$u_name][$network])) {
                                $this->log_message(
                                    "*** someone (network: $network) add us to their list (but already in our list): $u_name@$u_domain"
                                );
                            } else {
                                $re_login = false;
                                $cnt = 0;
                                foreach (array('Allow', 'Reverse') as $list) {
                                    if (! $this->addMemberToList($u_name . '@' . $u_domain, $network, $list)) {
                                        if ($re_login) {
                                            $this->log_message(
                                                "*** can't add $u_name@$u_domain (network: $network) to $list"
                                            );
                                            continue;
                                        }
                                        $aTickets = $this->get_passport_ticket();
                                ...
                                $this->log_message(
                                    "*** someone (network: $network) add us to their list: $u_name@$u_domain"
                                );
                            }
                            ...
                            $this->writeln("ADL $this->id $len");
                            $this->writedata($str);
                            if ($my_add_function && function_exists($my_add_function) && is_callable(
                                $my_add_function
                            )
                            ) {
                    ...
                    if (is_numeric($size) && $size > 0) {
                        ...
                        if (is_array($matches) && count($matches) > 0) {
                            ...
                            if (isset($aContactList[$u_domain][$u_name][$network])) {
                                ...
                                $this->log_message(
                                    "*** someone (network: $network) remove us from their list: $u_name@$u_domain"
                                );
                            } else {
                                $this->log_message(
                                    "*** someone (network: $network) remove us from their list (but not in our list): $u_name@$u_domain"
                                );
                            }
                            if ($my_rem_function && function_exists($my_rem_function) && is_callable(
                                $my_rem_function
                            )
                            ) {
                    ...
                    if (is_numeric($size) && $size > 0) {
                        ...
                        if (strcasecmp($maildata, 'too-large') == 0) {
                            ...
                            if ($maildata === false) {
                                ...
                                if ($maildata === false) {
                                    $this->log_message(
                                        "*** can't get mail-data via SOAP, and we already re-login again, so ignore this OIM"
                                    );
                                    break;
                                }
                            }
                        ...
                        foreach ($aOIMs as $maildata) {
                            ...
                            $this->log_message(
                                "*** You've OIM sent by $oim_sender, Time: $oim_time, MSGID: $oim_msgid, size: $oim_size"
                            );
                            ...
                            if ($sMsg === false) {
                                ...
                                if ($re_login) {
                                    $this->log_message(
                                        "*** can't get OIM via SOAP, and we already re-login again, so ignore this OIM"
                                    );
                                    continue;
                                }
                                $aTickets = $this->get_passport_ticket();
                            ...
                            if ($my_function && function_exists($my_function) && is_callable($my_function)) {
                                ...
                                if ($sMessage !== '') {
                                    ...
                                    $fname = dirname(
                                        $_SERVER['argv'][0]
                                    ) . DIRECTORY_SEPARATOR . 'spool' . DIRECTORY_SEPARATOR . 'msn_' . $this->getpid(
                                    ) . '_' . md5('offline' . rand(1, 1000) . $now) . '.msn';
                                    $fp = fopen($fname, 'wt');
                                    if ($fp) {
                                        ...
                                        chmod($fname, 0666);
                                        $this->log_message(
                                            "Response to $oim_sender (Offline, network: $network): $fname"
                                        );
                                    }
                                }
                            }
                ...
                case 'XFR':
                    ...
                    @list( /* XFR */, /* {id} */, $server_type, $server, /* CKI */, $cki_code, /* ... */) = @explode(
                        ' ',
                        $data
                    );
                    @list($ip, $port) = @explode(':', $server);
                    ...
                    if ($nCurrentUser < $nMSNUsers) {
                        ...
                        $this->writeln("XFR $this->id SB");
                        $this->debug_message(
                            "*** process " . $aMSNUsers[$nCurrentUser] . " ($nCurrentUser/$nMSNUsers)"
                        );
                        continue;
                    }
                    ...
                    if ($this->protocol == 'MSNP9') {
                        ...
                        $this->debug_message(
                            "*** MSNP9 don't support OIM, so we ignore offline and other network user!"
                        );
                        $this->log_message("*** already send message from $process_file");
                        if ($backup_file) {
                            $backup_dir = dirname($_SERVER['argv'][0]) . '/backup';
                    ...
                    foreach ($aOfflineUsers as $to) {
                        for ($i = 0; $i < $this->oim_try; $i ++) {
                            ...
                            if ($oim_result !== false && $oim_result['error'] === 'q0:SenderThrottleLimitExceeded') {
                                $this->log_message(
                                    "*** OIM failed for q0:SenderThrottleLimitExceeded, wait for " . $this->oim_throttle_delay . " seconds"
                                );
    ...
    function sendMessage($sMessage, $aTo)
    {
        ...
        while (! feof($this->fp)) {
            ...
            switch ($code) {
                ...
                case 'XFR':
                    // NS: <<< XFR {id} SB {server} CKI {cki} U messenger.msn.com 0
                    @list( /* XFR */, /* {id} */, /* SB */, $server, /* CKI */, $cki_code, /* ... */) = @explode(
                        ' ',
                        $data
                    );
                    @list($ip, $port) = @explode(':', $server);
                ...
                case 'MSG':
                    ...
                    if ($is_p2p) {
                        ...
                        if ($hdr_TotalDataSizeHigh == 0) {
                            ...
                            $totalsize = base_convert(
                                sprintf("%X%08X", $hdr_TotalDataSizeHigh, $hdr_TotalDataSizeLow),
                                16,
                                10
                            );
                            $dataoffset = base_convert(
                                sprintf("%X%08X", $hdr_DataOffsetHigh, $hdr_DataOffsetLow),
                                16,
                                10
                            );
...
' . chunk_split(base64_encode($sMessage)) . '
    ...
    function log_message($str)
    {
        ...
        $fname = $this->log_path . DIRECTORY_SEPARATOR . 'log' . DIRECTORY_SEPARATOR . 'msn_' . strftime(
            '%Y%m%d'
        ) . '.log';
        $fp = fopen($fname, 'at');
        if ($fp) {
