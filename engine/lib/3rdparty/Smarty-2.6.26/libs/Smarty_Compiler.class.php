...
class Smarty_Compiler extends Smarty
{
    ...
    function _compile_file($resource_name, $source_content, &$compiled_content)
    {
        ...
        if (count($this->_tag_stack) > 0) {
            ...
            $this->_syntax_error(
                "unclosed tag \{$_open_tag} (opened line $_line_no).",
                E_USER_ERROR,
                __FILE__,
                __LINE__
            );
        ...
        $template_header = "<?php /* Smarty version " . $this->_version . ", created on " . strftime(
            "%Y-%m-%d %H:%M:%S"
        ) . "\n";
    ...
    function _compile_block_tag($tag_command, $tag_args, $tag_modifier, &$output)
    {
        ...
        if ($start_tag) {
            ...
            $output .= '$_block_repeat=true;' . $this->_compile_plugin_call(
                'block',
                $tag_command
            ...
            $_out_tag_text = $this->_compile_plugin_call(
                'block',
                $tag_command
    ...
    function _compile_registered_object_tag($tag_command, $attrs, $tag_modifier)
    {
        ...
        if (! is_object($this->_reg_objects[$object][0])) {
            $this->_trigger_fatal_error(
                "registered '$object' is not an object",
                $this->_current_file,
    ...
    function _compile_insert_tag($tag_args)
    {
        ...
        if (! preg_match('~^\w+$~', $name)) {
            return $this->_syntax_error(
                "'insert: 'name' must be an insert function name",
                E_USER_ERROR,
                __FILE__,
                __LINE__
            );
            ...
            switch (strtolower($token)) {
                ...
                default:
                    if (preg_match('~^' . $this->_func_regexp . '$~', $token)) {
                        ...
                        '+-*/^%&|',
                        substr($token, - 1)
                    ) === false) && isset($tokens[$i + 1]) && $tokens[$i + 1] == '('
                    ) {
                        ...
                        '~^' . $this->_obj_call_regexp . '|' . $this->_var_regexp . '(?:' . $this->_mod_regexp . '*)$~',
                        $token
                    )
                    ) {
                        // object or variable
    ...
    function _parse_var_props($val)
    {
        ...
        if (preg_match(
            '~^(' . $this->_obj_call_regexp . '|' . $this->_dvar_regexp . ')(' . $this->_mod_regexp . '*)$~',
            $val,
            $match
        )
        ) {
    ...
    function _expand_quoted_text($var_expr)
    {
        ...
        if (preg_match_all(
            '~(?:\`(?<!\\\\)\$' . $this->_dvar_guts_regexp . '(?:' . $this->_obj_ext_regexp . ')*\`)|(?:(?<!\\\\)\$\w+(\[[a-zA-Z0-9]+\])*)~',
            $var_expr,
            $_match
        )
        ) {
    ...
    function _parse_var($var_expr)
    {
        ...
        if (! $_has_math) {
            ...
            preg_match_all(
                '~(?:^\w+)|' . $this->_obj_params_regexp . '|(?:' . $this->_var_bracket_regexp . ')|->\$?\w+|\.\$?\w+|\S+~',
                $_var_ref,
                $match
            );
    ...
    function _parse_modifiers(&$output, $modifier_string)
    {
        ...
        for ($_i = 0, $_for_max = count($_modifiers); $_i < $_for_max; $_i ++) {
            ...
            if ($_map_array) {
                $output = "((is_array(\$_tmp=$output)) ? \$this->_run_mod_handler('$_modifier_name', true, \$_tmp$_modifier_args) : " . $this->_compile_plugin_call(
                    'modifier',
                    $_modifier_name
                ) . "(\$_tmp$_modifier_args))";
    ...
    function _compile_smarty_ref(&$indexes)
    {
        ...
        foreach ($indexes as $_index_no => $_index) {
            if (substr($_index, 0, 1) != '.' && $_index_no < 2 || ! preg_match('~^(\.|\[|->)~', $_index)) {
                $this->_syntax_error(
                    '$smarty' . implode('', array_slice($indexes, 0, 2)) . ' is an invalid reference',
                    E_USER_ERROR,
                    __FILE__,
                    __LINE__
                );
