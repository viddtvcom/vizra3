<h2 class="title700">##TopMenu%Finance##</h2>


<ul class="s_tabb clear">
    <li><a {if $tab == 'bills' || $tab == ''}class="selected"{/if}
           href="{$vurl}?p=user&s=finance&tab=bills">##Bills##</a></li>
    <li><a {if $tab == 'payments'}class="selected"{/if} href="{$vurl}?p=user&s=finance&tab=payments">##Payments##</a>
    </li>
    <li><a {if $tab == 'accflow'}class="selected"{/if} href="{$vurl}?p=user&s=finance&tab=accflow">##AccountFlow##</a>
    </li>
...
{if $tab == 'bills' || $tab == ''}
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="main_table">
        <tr>
            <th width="20"></th>
            <th>##PackageName##
            <th class="tright">##DueDate##</th>
            <th width="80" class="tright">##Amount##</th>
        </tr>
        {foreach from=$bills item=b}
            <tr {cycle values=",class='alt'"}>
                ...
                <td class="tright">{format_date date=$b.dateDue}</td>
                <td class="tright">{$b.amount} {getCurrencyById id=$b.paycurID}</td>
            </tr>
        {/foreach}
    </table>
{elseif $tab == 'payments'}
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="main_table">
        <tr>
            ...
            <th width="50" class="tright">##Amount##</th>
        </tr>
        {foreach from=$payments item=p}
            <tr {cycle values=",class='alt'"}>
                <td><img src="images/ico_{if $p.paymentStatus == "paid"}active{else}pending{/if}.png" width="13"></td>
                ...
                <td class="tright">{$p.amount} {getCurrencyById id=$p.paycurID}</td>
            </tr>
    ...
    {if $chart}
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="main_table">
            <tr>
                ...
                <th>##Description##</th>
                <th width="80" class="tright">##Amount##</th>
                <th width="80" class="tright">##Balance##</th>
            </tr>
            {foreach from=$chart item=c}
                <tr {cycle values=",class='alt'"}>
                    <td>{format_date date=$c.timestamp mode=date type=short}</td>
                    <td class="bold {if $c.type == "bill"}pink{else}blue{/if}"
                        width="13">{if $c.type == "bill"}##Bill##{else}##Payment##{/if}</td>
                    <td>{$c.description}</td>
                    <td class="bold tright {if $c.type == "bill"}pink{else}blue{/if}">{$c.amount} {getCurrencyById id=$c.paycurID}</td>
                    <td class="bold tright {if $c.balance < 0}pink{else}blue{/if}">{$c.balance|number_format:2} {getCurrencyById id=$smarty.const.MAIN_CUR_ID}</td>
                </tr>
            {/foreach}
            <tr class='alt2'>
                <td class="bold tright" colspan="4">Bakiye:</td>
                <td class="bold tright {if $c.balance < 0}pink{else}blue{/if}">{$c.balance|number_format:2} {getCurrencyById id=$smarty.const.MAIN_CUR_ID}</td>
            </tr>
        </table>
