<h2 class="title700"><img src="{$turl}images/{$Order->getStatusIcon()}" class="middle" width="13"> {$Order->title}</h2>


<ul class="s_details">

    <li class="left tleft">
        <span style="color:#666;">##OrderDetails%ORDERNUMBER## #{$Order->orderID} / ##OrderDetails%ORDERSTATUS##:</span>
        <span class="durum">##OrderDetails%{$Order->status}##</span><br/>
...
<ul class="s_tabb clear">
    ...
    {else}
        <li><a {if $tab == 'domain'}class="selected"{/if}
               href="{$vurl}?p=user&s=orders&a=details&oID={$Order->orderID}&tab=domain">##DomainManagement##</a></li>
    {/if}
...
<div class="content_right clear">
    ...
    {elseif $tab == 'addons'}
        ...
        {if $addonServices}
            <div class="addon_select" style="text-align: right; padding: 3px 0;">
                ...
                <input type="submit" value="##Add##" id='addonSubmit' class="button">
            </div>
            {literal}
