...
<form action="{$vurl}?p=cart&s=srvconf&a=update&key={$smarty.get.key}" method="post" id="formgrid" style="width:100%;">
    {if $smarty.get.a == 'add'}
        <input type="hidden" name="serviceID" value="{$Service->serviceID}">
    {/if}
    <ul>
        {if $Service->setup > 0}
            <li><label>##Setup##</label>{$Service->setup-$Service->setup_discount} {$Service->paycurID|getCurrencyById}
            </li>
        {/if}
        ...
        <li>
            ...
            <textarea name="description" class="tinput" rows="3" style="width:400px;">{$cartItem.description}</textarea>
        </li>
    </ul>
    ...
    {if $dom}
        ...
        {else}
            <ul id="domcontainer" {if $smarty.get.a == 'update-form'}style="display:none;"{/if}>
                <li><input class="domreg_radio" type="radio" name="domreg" id="domreg_new" value="new"
                           checked="checked"> ##IWantToRegisterThisDomain##
                </li>
                <li><input class="domreg_radio" type="radio" name="domreg" id="domreg_existing" value="existing">
                    ##IWillUseMyExistingDomain##
                </li>
                <li style="text-align: center;">
                    ...
                    <input type="submit" value="##Check##" class="button" id="but_cont">
                </li>
                <li></li>
            </ul>
        {/if}
    ...
    {if $addons}
        ...
        <ul>
            {foreach from=$addons item=a}
                <li>
                    <input type="checkbox" name="addons[{$a.serviceID}]" value="1"> {$a.service_name} -
                    ({$a.price|displayPrice:$a.period:$a.paycurID})
                </li>
            {/foreach}
        </ul>
    {/if}
    ...
    <p align="right">
        {if $smarty.get.a == 'add'}
            <input type="submit" value="##AddToCart##" class="button" style="display:{if $dom}none{/if};"
                   id="but_submit">
        {else}
            <input type="submit" value="##Update##" class="button" id="but_submit">
        {/if}
    </p>
</form>
