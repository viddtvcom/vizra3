<h2 class="title2">##Nav%DomainRegistration##</h2>
<div class="article">
    ...
    <form action="{$vurl}?p=shop&s=domain&a=check" method="post">{$formToken}
        ...
        <textarea name="domain" id="domain" rows="2" cols="5" class="tinput left br_5" style="width:616px;"
                  onkeyup="return taCount('domain','NODISPLAY');"></textarea>
        <br class="clear"/>
        <input type="submit" class="button clear right br_5" style="margin-top:5px;" value="##Search##"/>
        <ul class="domain_uzantilari" style="width: 580px; float: left; margin:5px 0 0 50px;">
            {foreach from=$extensions key=ext item=i}
                <li><label><input type="checkbox" name="ext[]" value="{$ext}" checked>.{$ext}</label></li>
            {/foreach}
        </ul>
    ...
    {if $post}
                        ...
                        $.post(vurl + "?p=ajax", { action: "lookup_domain", domain: $domain, key: $k },
                                function (ret) {
                                    ...
                                    if (ret.st) {
                                        ...
                                        $("#check_" + ret.key).html('<img src="images/stop-alt.png">');
                                    }
                                }, "json");
        ...
        <form action="{$vurl}?p=cart&s=domconf" method="post">
            ...
            <p align="right"><input type="submit" value="##AddToCart##" class="button br_5"
                                    onclick="return checkboxes();"></p>
